const { trainerInfo, TrainerTable, LearnsetTable, moveEnum } = require('../../lumibot/__gamedata');
...
function generate_moves_via_learnset(mons_no, level) {
  ...
  */
  if (!Number.isInteger(mons_no) || mons_no < 0 || !LearnsetTable['WazaOboe'][mons_no]) {
  ...
  const cutoff_index = LearnsetTable['WazaOboe'][mons_no]['ar'].findIndex((move, index) => index % 2 === 0 && move > level);
