const { abilityNames, abilityInfo } = require(global.gameDataFolder);

function makeSmogonAbilityObject(abilityId = 0) {
	const abilityString =
		abilityNames.labelDataArray[abilityId]?.wordDataArray[0]?.str ?? null;
...
function getAbilityString(abiltiyId = 0) {
	const abilityName =
		abilityNames.labelDataArray[abiltiyId]?.wordDataArray[0]?.str ?? null;
...
function getAbilityInfo(id) {
	...
	if (wordData === null || wordData === undefined || wordData.length === 0)
		return "None";
