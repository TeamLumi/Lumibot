const { pokemonHeight, pokemonWeight } = require(global.gameDataFolder);
...
function getHeight(pokemonId = 0) {
	const heightString =
		pokemonHeight.labelDataArray[pokemonId]?.wordDataArray[0]?.str ?? null;
...
function getWeight(pokemonId = 0) {
	const weightString =
		pokemonWeight.labelDataArray[pokemonId]?.wordDataArray[0]?.str || null;
